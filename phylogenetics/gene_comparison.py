...
def plot_heatmap(df, outpath):
    ...
    species_labels = [n.split("_copy")[0] for n in df.index]
...
def get_display_order(species):
    ...
    - Arrange scaffolds with ASCT3's scaffold first, then others
    ...
    for scaf in scaffolds:
        ...
        has_asct3 = any("ASCT3" in n for n in names)
        asct3_first = names[0] if has_asct3 else None
        if has_asct3 and "ASCT3" not in names[0]:
            scaffolds[scaf].reverse()
        elif not has_asct3 and all(s == "-" for s in strands):
            scaffolds[scaf].reverse()

    def scaffold_priority(scaf):
        best = 9
        for e in scaffolds[scaf]:
            num = e["name"].split("_ASCT")[1][0]
            best = min(best, {"3": 0, "2": 1, "1": 2}.get(num, 9))
        return (best, scaf)
