#!/bin/bash
#SBATCH -J kwasiblastexons		# jobname
#SBATCH -o kwasiblastexons.o%A.%a	# jobname.o%j for single (non array) jobs jobname.o%A.%a for array jobs
#SBATCH -e kwasiblastexons.e%A.%a	# error file name A is the jobid and a is the arraytaskid
#SBATCH -a 1 #-7			# start and stop of the array start-end
...
outfmttype="6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen qcovs"
bestn=5

#JOBFILE=exon_queries.txt
JOBFILE=TE_queries.txt
...
# Outgroups
##MYLABEL="Mytilisepta_virgata"
##MYDB"GCA_028015205.1"
##MYLABEL="Limnoperna_fortunei"
##MYDB="GCA_944474755.1"
##MYLABEL="Bathymodiolus_septemdierum"
##MYDB="GCA_963383655.1"
##MYLABEL="Bathymodiolus_brooksi"
##MYDB="GCA_963680875.1"
##MYLABEL="Bathymodiolus_thermophilus"
##MYDB="GCA_971856555.1"
##MYLABEL="Bathymodiolus_heckerae"
##MYDB="GCA_965117275.1"
...
#blastn -query ${QUERY} -db $(basename ${MYDB} | sed 's/\.\(fasta\|fna\|fa\)$//')  -outfmt "${outfmttype}" -evalue 1e6 -task blastn -word_size 20 -dust no -num_threads ${SLURM_CPUS_PER_TASK} > $(basename ${QUERY} | sed 's/\.\(fasta\|fna\|fa\)$//')_$(basename ${MYDB} | sed 's/\.\(fasta\|fna\|fa\)$//')_e6p_blastn.outfmt6
blastn -query ${QUERY} -db $(basename ${MYDB} | sed 's/\.\(fasta\|fna\|fa\)$//')  -outfmt "${outfmttype}" -evalue 1e-5 -num_threads ${SLURM_CPUS_PER_TASK} > $(basename ${QUERY} | sed 's/\.\(fasta\|fna\|fa\)$//')_$(basename ${MYDB} | sed 's/\.\(fasta\|fna\|fa\)$//')_e5_blastn.outfmt6
