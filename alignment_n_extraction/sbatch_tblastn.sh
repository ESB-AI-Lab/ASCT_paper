...
#SBATCH -p p1,bigmemgc			# queue (partition) -- normal, development, largemem, etc.
...
QUERY=APSCT_FASHE_C6EUD4_swissprot.fa
...
tblastn -query ${QUERY} -db $(basename ${MYDB} | sed 's/\.\(fasta\|fna\|fa\)$//')  -outfmt "${outfmttype}" -evalue 1e-5 -num_threads ${SLURM_CPUS_PER_TASK} > $(basename ${QUERY} | sed 's/\.\(fasta\|fna\|fa\)$//')_$(basename ${MYDB} .fasta)_e5_blastn.outfmt6
#tblastn -query ${QUERY} -db $(basename ${MYDB} .fasta)  -max_target_seqs ${bestn} -outfmt $outfmttype -evalue 1 -num_threads ${SLURM_CPUS_PER_TASK} > $(basename ${QUERY} .fasta)_$(basename ${MYDB} .fasta)_e1_blastn_best${bestn}.outfmt6
